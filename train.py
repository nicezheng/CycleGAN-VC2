...
class CycleGANTraining(object):
    ...
    def train(self):
        # Training Begins
        for epoch in tqdm(range(self.start_epoch, self.num_epochs), desc=f"Epoch"):
            ...
            if epoch % self.save_epoch_freq == 0 and epoch != 0:
                end_time = time.time()
                store_to_file = "Epoch: {} Generator Loss: {:.4f} Discriminator Loss: {}, Time: {:.2f}\n\n".format(
                    epoch, generator_loss.item(), d_loss.item(), end_time - start_time_epoch)
                self.store_to_file(store_to_file)
    ...
    def validation_for_A_dir(self, epoch):
        ...
        output_dir = os.path.join(output_A_dir, str(epoch).zfill(5))
    ...
    def validation_for_B_dir(self, epoch):
        ...
        output_dir = os.path.join(output_B_dir, str(epoch).zfill(5))
        if not os.path.exists(output_dir):
            os.makedirs(output_dir, exist_ok=True)
...
if __name__ == '__main__':
    ...
    validation_A_dir_default = './data/jz/'
    output_A_dir_default = './converted_sound/jz'

    validation_B_dir_default = './data/jay/'
    output_B_dir_default = './converted_sound/jay/'
